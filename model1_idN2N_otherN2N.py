"""
Model1 embed all AST nodes according to their type name.
An end-to-end(N2N) way is used to embed each type of AST node.
We randomly initialize the embedding for each AST node type and learn the embeddings of the AST node during the training.
"""
...
def train_model(embeddings):
    ...
    learn_rate = tf.train.exponential_decay(
        LEARN_RATE, global_step, len(TrainDatalist), 0.9, True)
    ...
    for global_step in range(1, EPOCHS + 1):
        ...
        for line in TrainDatalist:
            ...
            if (train_data[0] in listrec) or (train_data[1] in listrec):
                continue
            nodes11, children1, nodes22, children2, batch_labels = getData_notokenfinetune(
    ...
    for name in test_list:
        ...
        ff = open("dataset/dataset/test/" + name +
                  "/" + data_type + ".txt", 'r')
        line = "123"
        k = 0
        while line:
            ...
            if (test_data[0] in listrec) or (test_data[1] in listrec):
                continue
            nodes11, children1, nodes22, children2, _ = getData_notokenfinetune(
            ...
            for i in range(0, 20):
                ...
                threaholder += 0.1
            if global_step == 10:
                with open("dataset/cluster/1/" + name + "/" + test_data[0].split('_')[0] + '_' + test_data[1].split('_')[0] + '.txt', 'a') as fout:
                    fout.writelines(
                        test_data[2] + '\t' + test_data[0] + '\t' + test_data[1] + '\t' + str(output[0]) + '\n')
        ...
        accuracy = accuracy_score(
            correct_labels_test, predictions_test[maxstep])
        ...
        if name == "all":
            with open("dataset/cluster/1/total_result.csv", 'a') as total_result:
...
if __name__ == '__main__':
    ...
    for l in listchar:
        listta.append(np.random.normal(
            0, 0.1, feature_size).astype(np.float32))
